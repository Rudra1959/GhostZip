with open("c:\\Users\\HP\\Videos\\Ghostzip\\src-tauri\\src\\engine\\extract.rs", "w") as f:
    f.write(r'''use std::fs::{self, File};
...
pub fn extract_session(
    ...
    session.space_saver = match selected_mode {
        ...
        UserExtractionMode::SpaceSaver => true,
    };
    ...
    match session.manifest.format {
        ...
        other => Err(EngineError::UnsupportedFormat(other.label().to_string())),
    }
}
